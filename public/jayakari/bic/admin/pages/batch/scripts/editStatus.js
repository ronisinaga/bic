...
var batch = {
        ...
        toastr.options = {
            ...
            "onHidden": function() {
                if (batch.config.valid){
                    batch.redirectToPage(host+'/admin/batch/status');
                }
            },
            "showDuration": "300",
            "hideDuration": "1000",
    ...
    valid: function(){
        ...
        if ($('#selStatus').val() == ''){
            toastr['error']('Status batch harus dipilih','Error');
    ...
    save: function(json){
        if (batch.valid()){
            $.ajax({
                method: 'POST',
                url: host+'/admin/batch/status/'+$('#id').val()+'/edit',
                data: {
                    data: json
                },
                dataType: "json",
...
$('#btnSimpan').on('click',function(){
    var json= '{"sender":"bic","id":'+$('#id').val()+',"status":'+$('#selStatus').val()+'}';
    batch.save(json);
});
