...
var index = {
    ...
    hapus: function(json){
        ...
        $.confirm({
            title: 'Hapus Blog',
            content: 'Apakah anda ingin menghapus blog ini? Apabila anda hapus seluruh data yang berkaitan dengan data blog ini akan dihapus juga',
            buttons: {
                confirm: {
                    text: "Hapus",
                    action: function () {
                        ...
                        $.ajax({
                            ...
                            success: function (response){
                                if (response.status == 'success'){
                                    ...
                                    index.config.valid = false;
                                    toastr['error']("Sistem gagal hapus blog","Kesalahan");
                                }
                            },
                            error: function (response) {
                                //alert('Errors');
...
$('#new').on('click',function(){
    index.redirectToPage(host+'/admin/blog/create');
});
...
$('#sample_1').on('click','#hapus',function(){
    var tr = $(this).closest('tr');
    var td = $(tr).find('td');
    var arr = {
        'sender':'BIC',
