...
var berita = {
    ...
    validasi: function(){
        ...
        if ($('#file').get(0).files.length == 0){
            toastr['error']('file untuk proposal harus ada','Error');
    ...
    removeFile: function(json){
        $.ajax({
            ...
            success: function (response){
                if (response.status == 'success'){
                    berita.config.valid = true;
                    toastr['success']("Sistem berhasil hapus file proposal","Success");
                }else{
                    berita.config.valid = false;
                    toastr['success']("Sistem gagal hapus file proposal. Hubungi administrator anda","Error");
                }
            },
            error: function (response) {
                berita.config.valid = false;
...
$('#btnBatal').on('click',function(){
    berita.redirectToPage(mainback);
});
