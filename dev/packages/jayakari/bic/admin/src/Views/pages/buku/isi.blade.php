                                ...
                                @foreach($proposal as $data)
                                    <tr>
                                        ...
                                        <?php
                                            if ($data->isiBuku == null){
                                                if ($activeCategory == 7){
                                                    echo '<td><a href="'.env('APP_URL').'/admin/buku/'.$buku->id.'/'.$data->id.'/buatisibuku" class="btn blue"><i class="fa fa-plus"></i> Isi Buku </a></td>';
                                                }else{
                                                    echo '<td><a href="'.env('APP_URL').'/admin/buku/'.$buku->id.'/'.$data->id.'/buatisibuku" class="btn red"><i class="fa fa-view"></i> Isi Buku </a></td>';
                                                }
                                            }else{
                                                if ($activeCategory == 7){
                                                    echo '<td width="380px">';
                                                    echo '<a href="'.env('APP_URL').'/admin/buku/'.$data->isiBuku->id.'/editisibuku" class="btn blue"><i class="fa fa-pencil"></i> Edit </a>';
                                                    echo '<a href="'.env('APP_URL').'/admin/buku/'.$data->isiBuku->id.'/file" class="btn green"><i class="fa fa-plus"></i> File </a>';
                                                    echo '<a href="'.env('APP_URL').'/admin/buku/'.$data->isiBuku->id.'/ebook" class="btn yellow"><i class="fa fa-plus"></i> Ebook </a>';
                                                    echo '<a href="'.env('APP_URL').'/admin/buku/'.$data->isiBuku->id.'/video" class="btn purple"><i class="fa fa-plus"></i> Video </a>';
