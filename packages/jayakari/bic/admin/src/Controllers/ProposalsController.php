...
class ProposalsController extends Controller
{
    ...
    public function lengkapi($idProposal){
        ...
        if (count($user->get()) == 0){
            ...
            if ($filled){
                $num = count($proposal->kunciKolaborasi);
                if ($num == 0){
                    $filled = false;
                }
            }
    ...
    public function editProposal($idProposal,$tab){
        ...
        if (count($user->get()) == 0){
            ...
            //inovator member
            $inovatorMember = InovatorMember::where('id_inovator',$user->get()[0]->id)->get();
            $proposalInovatorMember = array();
            foreach($inovatorMember as $item){
                $pim = ProposalInovatorMember::where([
                    'id_inovator_member'=>$item->id,
                    'id_proposal'=>$proposal[0]->id
                ])->get();
                if (count($pim) > 0){
                    $item->id_rsc = $pim[0]->id_rsc;
                    $item->rsc_name = $pim[0]->rsc->rsc;
                    $proposalInovatorMember[] = $item;
                }
            }
            ...
            for($i=0;$i<$num;$i++){
                ...
                $proposal[$i]->abstrak = str_replace(array("\r\n","\r","\n"), '\n', str_replace('"', '\"', addcslashes(str_replace("\r", '', (string)$proposal[$i]->abstrak), "\0..\37'\\")));
                $proposal[$i]->deskripsi = str_replace(array("\r\n","\r","\n"), '\n', str_replace('"', '\"', addcslashes(str_replace("\r", '', (string)$proposal[$i]->deskripsi), "\0..\37'\\")));
                $proposal[$i]->keunggulan_teknologi = str_replace(array("\r\n","\r","\n"), '\n', str_replace('"', '\"', addcslashes(str_replace("\r", '', (string)$proposal[$i]->keunggulan_teknologi), "\0..\37'\\")));
                $proposal[$i]->potensi_aplikasi = str_replace(array("\r\n","\r","\n"), '\n', str_replace('"', '\"', addcslashes(str_replace("\r", '', (string)$proposal[$i]->potensi_aplikasi), "\0..\37'\\")));
            }
            ...
            if (Cookie::has('active_category')){
                return view('jayakari.bic.admin::pages.proposals.edit',
                    [
                        ...
                        "employee"=>$employee,
                        "instansi"=>$instansi,
                        "inovatormember"=>$proposalInovatorMember,
                        "tab"=>$tab,
                        "datauser" => $user->get(),
                        "activeCategory"=>Cookie::get('active_category'),
                        'kategorilabel'=>$this->kategorilabel
    ...
    public function review($id){
        ...
        if (count($user->get()) == 0){
            ...
            $proposalJuri = ProposalJuri::where('id_proposal',$id)->get();
            $num = count($proposalJuri);
            for($i=0;$i<$num;$i++){
                $batch = "";
                $topikstr = "";
                $topik = Topik::where('id',$proposalJuri[$i]->id_topik)->get();
                if (count($topik) > 0){
                    $batch = $topik[0]->batch->batch;
                    $topikstr = $topik[0]->topik;
                }
                $proposalJuri[$i]->batch = $batch;
                $proposalJuri[$i]->topikstr = $topikstr;
            }
            if (Cookie::has('active_category')){
    ...
    public function detailMasuk($id,$tahapan){
        ...
        if (count($user->get()) == 0){
            ...
            if($proposal[0]->instansi <> null){
                ...
                $inovasimember = ProposalInovatorMember::where('id_proposal',$proposal[0]->id)->get();
            }else{
                $bidangusaha = array();
                $employee = "";
            }
