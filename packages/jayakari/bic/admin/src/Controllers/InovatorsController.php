...
class InovatorsController extends Controller
{
    ...
    public function video(Request $request){
        ...
        if (count($user->get()) == 0){
            ...
            if (Cookie::has('active_category')){
                return view('jayakari.bic.admin::pages.inovators.video', [
                    "videos" => $videos,
                    "datauser" => $user->get(),
    ...
    public function videoAdd(Request $request){
        ...
        if (count($user->get()) == 0){
            ...
            $innovator = DB::select( DB::raw("select bu.id,bu.fullname from (select buku.id_user,buku.id_user_kategori from (SELECT id FROM `bic_user_kategori` where id=4) buk inner join bic_user_kategori_user buku on buk.id = buku.id_user_kategori) buku inner join bic_user bu on bu.id = buku.id_user; ") );
            if (Cookie::has('active_category')){
                return view('jayakari.bic.admin::pages.inovators.videoAdd', [
                    "innovator" => $innovator,
                    "datauser" => $user->get(),
                    'activeCategory'=>Cookie::get('active_category'),
                    'kategorilabel'=>$this->kategorilabel
    ...
    public function videoSave(Request $request){
        ...
        if (count($user->get()) == 0){
            ...
            VideoInnovator::create([
                'id_innovator' => $json->id_innovator,
                'title' => $json->title,
                'url_youtube' => $json->url,
    ...
    public function videoEdit($id){
        ...
        if (count($user->get()) == 0){
            ...
            $innovator = DB::select( DB::raw("select bu.id,bu.fullname from (select buku.id_user,buku.id_user_kategori from (SELECT id FROM `bic_user_kategori` where id=4) buk inner join bic_user_kategori_user buku on buk.id = buku.id_user_kategori) buku inner join bic_user bu on bu.id = buku.id_user; ") );
            $videos= VideoInnovator::where('id', $id)->get();
            if (Cookie::has('active_category')){
                return view('jayakari.bic.admin::pages.inovators.videoEdit', [
                    'videos' => $videos,
                    "innovator" => $innovator,
                    'datauser' => $user->get(),
                    'activeCategory'=>Cookie::get('active_category'),
    ...
    public function videoUpdate(Request $request){
        ...
        if (count($user->get()) == 0){
            ...
            $data = $request->input('data');
            $json = json_decode($data);
            $vi = VideoInnovator::where('id',$json->id);
            if (count($vi->get()) > 0){
                $vi->update([
                    'id_innovator' => $json->id_innovator,
                    'title' => $json->title,
